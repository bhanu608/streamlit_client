...
if not pending_requests:
    ...
    for request in pending_requests:
        with st.expander(request["id"]):
            st.write(f"Action: {request['action']}")
            # st.write(f"resource_name: {request.get('resource_name', 'N/A')}")
            ...
            with col1:
                if st.button("Approve", 
                    ...
                    requests.put(f"http://localhost:9001/request/{request['id']}", json={"status": "approved"})
